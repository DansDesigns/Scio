...
def Atmos():
    ...
    # change io.BytesIO(<TARGET>) to which ever sensor
    # add if statements for easy sensor changing without changing whole screen
    plot_stream = io.BytesIO(temperature)
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~ SAVE SENSOR DATA ~~~~~~~~~~~~~~~~~~~~~~~~
    # ENTER/SELECT/OK (Single Button Save to file Function):
    ...
    # add record for time function
    # ~~~~~~~~~~~~~~~~~~~~~~~~ SAVE SENSOR DATA 10s ~~~~~~~~~~~~~~~~~~~~~~~~
    # Exit/Back/Other (Single Button Save to file Function - every 1s for 10s):
    if GPIO.input(button_4) == GPIO.LOW:
        a = ("Temp: " + f"{temperature}" + "*C | ")
        b = ("Humidity: " + f"{humidity}" + "%RH | ")
        c = ("Pressure: " + f"{pressure}" + "hPa | ")
        d = ("InnerTemp: " + f"{mpu_temp}" + "*C | ")
        save_to_log10s(a, b, c, d)
...
def Wifi_feed():
    ...
    if server_state == "Active":
        server_state_colour = green
    if server_state == "Inactive":
        server_state_colour = red

    draw_text2("WiFi Sensor:", 18, white, 0, 8)
    draw_text2(f"{server_state}", 16, server_state_colour, 5, 62)
    draw_text2(f"{ip_i}", 16, blue, 5, 102)
